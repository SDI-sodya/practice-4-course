				...
				<div class="w70 a1">
					<div class="flex cubes">
						...
						<div class="w30 ctr">
							...
							data.items[iChart].views).toFixed(2) }} %
						</div>
					</div>
				</div>
			</div>
			...
			<div class="flex body">
				<div class="w30 ar filchart">
					<div class="itemchart ptb10" v-if="all">
						<toogle
							v-model="all"
							@update:modelValue="all = $event; checkAll(all)"
						/>
						All
					</div>
              ...
              "
				...
				<div class="w70" id="chartOuter"></div>
			</div>
		</popup>
		...
		>
			<div class="form inner-form">
				<form @submit.prevent="action()" v-if="parent.formData">
					...
					<div class="rows">
						<button class="btn" v-if="parent.formData && parent.formData.id">
							Edit
						</button>
					...
					<tbody>
						<tr v-for="(item, i) in data.items" :key="item.id">
							...
							<td class="id">
								<a
									href="#"
									@click.prevent="$refs.details.active = 1; getDetails(item.id, 1)"
								>
									{{ item.views }}
								</a>
							</td>
							<td class="id">
								<a
									href="#"
									@click.prevent="$refs.details.active = 1; getDetails(item.id, 2)"
								>
							...
							<td class="id">
								<a
									href="#"
									@click.prevent="$refs.details.active = 1; getDetails(item.id, 3)"
								>
							...
							<td class="id">
								<a
									href="#"
									@click.prevent="$refs.details.active = 1; getDetails(item.id, 4)"
								>
